/*
 * @Description: PCF8563 系列实时时钟芯片驱动实现
 * @Author: LILYGO_L
 * @Date: 2025-01-14 14:12:51
 * @LastEditTime: 2026-08-03 16:11:18
 * @License: GPL 3.0
 */
#include "chip/i2c/pcf8563x.h"

namespace cpp_bus_driver {
bool Pcf8563x::Init(int32_t freq_hz) {
  if (rst_ != kPinNotConnected) {
    bool result = true;
    result &= SetGpioMode(rst_, GpioMode::kOutput, GpioStatus::kPullup);
    result &= GpioWrite(rst_, 0);
    DelayMs(10);
    result &= GpioWrite(rst_, 1);
    DelayMs(10);
    if (!result) {
      LogMessage(LogLevel::kError, __FILE__, __LINE__, "Rst failed\n");
      return false;
    }
  }

  if (!I2cChipBase::Init(freq_hz)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__,
        "Init failed\n");
    return false;
  }

  auto buffer = GetChipId();
  if (buffer == kInvalidChipId) {
    LogMessage(LogLevel::kInfo, __FILE__, __LINE__,
        "Get pcf8563x chip id failed (error id: %#X)\n", buffer);
    return false;
  } else {
    LogMessage(LogLevel::kInfo, __FILE__, __LINE__,
        "Get pcf8563x chip id success (id: %#X)\n", buffer);
  }

  if (!InitSequence(kInitSequence, sizeof(kInitSequence))) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__,
        "InitSequence failed\n");
    return false;
  }

  ClearClockIntegrityFlag();

  return true;
}

bool Pcf8563x::Deinit(bool delete_bus) {
  bool result = true;

  if (!I2cChipBase::Deinit(delete_bus)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__,
        "Deinit failed\n");
    result = false;
  }

  if (rst_ != kPinNotConnected) {
    result &= ResetGpio(rst_);
  }

  return result;
}

uint8_t Pcf8563x::GetChipId() {
  uint8_t buffer = 0;

  if (!ReadRegister(static_cast<uint8_t>(Register::kRoChipId), &buffer)) {
    return kInvalidChipId;
  }

  return buffer;
}

bool Pcf8563x::SetClockFrequencyOutput(OutFreq freq_hz) {
  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwClkoutControl), &buffer)) {
    return false;
  }
  switch (freq_hz) {
    case OutFreq::kClockOff:
      buffer &= 0B01111111;
      break;
    case OutFreq::kClock1Hz:
      buffer |= 0B10000011;
      break;
    case OutFreq::kClock32Hz:
      buffer = (buffer & 0B01111100) | 0B10000010;
      break;
    case OutFreq::kClock1024Hz:
      buffer = (buffer & 0B01111100) | 0B10000001;
      break;
    case OutFreq::kClock32768Hz:
      buffer = (buffer & 0B01111100) | 0B10000000;
      break;

    default:
      break;
  }
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwClkoutControl), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetClock(bool enalbe) {
  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus1), &buffer)) {
    return false;
  }
  if (enalbe) {
    buffer &= 0B11011111;  // 开启时钟
  } else {
    buffer |= 0B00100000;  // 停止时钟
  }

  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus1), buffer)) {
    return false;
  }
  return true;
}

bool Pcf8563x::CheckClockIntegrityFlag() {
  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), &buffer)) {
    return false;
  }

  return !((buffer & 0B10000000) >> 7);
}

bool Pcf8563x::ClearClockIntegrityFlag() {
  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), &buffer)) {
    return false;
  }

  buffer &= 0B01111111;

  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), buffer)) {
    return false;
  }

  return true;
}

uint8_t Pcf8563x::GetSecond() {
  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), &buffer)) {
    return -1;
  }

  return (((buffer & 0B01110000) >> 4) * 10) + (buffer & 0B00001111);
}

uint8_t Pcf8563x::GetMinute() {
  uint8_t buffer = 0;

  if (!ReadRegister(static_cast<uint8_t>(Register::kRwMinutes), &buffer)) {
    return -1;
  }

  return (((buffer & 0B01110000) >> 4) * 10) + (buffer & 0B00001111);
}

uint8_t Pcf8563x::GetHour() {
  uint8_t buffer = 0;

  if (!ReadRegister(static_cast<uint8_t>(Register::kRwHours), &buffer)) {
    return -1;
  }

  return (((buffer & 0B00110000) >> 4) * 10) + (buffer & 0B00001111);
}

uint8_t Pcf8563x::GetDay() {
  uint8_t buffer = 0;

  if (!ReadRegister(static_cast<uint8_t>(Register::kRwDays), &buffer)) {
    return -1;
  }

  return (((buffer & 0B00110000) >> 4) * 10) + (buffer & 0B00001111);
}

uint8_t Pcf8563x::GetWeek() {
  uint8_t buffer = 0;

  if (!ReadRegister(static_cast<uint8_t>(Register::kRwWeekdays), &buffer)) {
    return -1;
  }

  return (buffer & 0B00000111);
}

uint8_t Pcf8563x::GetMonth() {
  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwCenturyMonths), &buffer)) {
    return -1;
  }

  return (((buffer & 0B00010000) >> 4) * 10) + (buffer & 0B00001111);
}

uint8_t Pcf8563x::GetYear() {
  uint8_t buffer = 0;

  if (!ReadRegister(static_cast<uint8_t>(Register::kRwYears), &buffer)) {
    return -1;
  }

  return (((buffer & 0B11110000) >> 4) * 10) + (buffer & 0B00001111);
}

bool Pcf8563x::GetTime(Time& time) {
  uint8_t buffer[7] = {0};

  for (uint8_t i = 0; i < 7; i++) {
    if (!ReadRegister(static_cast<uint8_t>(
                              static_cast<uint8_t>(Register::kRwVlSeconds) + i),
            &buffer[i])) {
      return false;
    }
  }

  time.second =
      (((buffer[0] & 0B01110000) >> 4) * 10) + (buffer[0] & 0B00001111);
  time.minute =
      (((buffer[1] & 0B01110000) >> 4) * 10) + (buffer[1] & 0B00001111);
  time.hour = (((buffer[2] & 0B00110000) >> 4) * 10) + (buffer[2] & 0B00001111);
  time.day = (((buffer[3] & 0B00110000) >> 4) * 10) + (buffer[3] & 0B00001111);
  time.week = static_cast<Week>(buffer[4] & 0B00000111);
  time.month =
      (((buffer[5] & 0B00010000) >> 4) * 10) + (buffer[5] & 0B00001111);
  time.year = (((buffer[6] & 0B11110000) >> 4) * 10) + (buffer[6] & 0B00001111);

  return true;
}

bool Pcf8563x::SetSecond(uint8_t second) {
  if (second > 59) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    second = 59;
  }

  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), &buffer)) {
    return false;
  }

  buffer = (buffer & 0B10000000) | (((second / 10) << 4) | (second % 10));

  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetMinute(uint8_t minute) {
  if (minute > 59) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    minute = 59;
  }

  uint8_t buffer = (((minute / 10) << 4) | (minute % 10));

  if (!WriteRegister(static_cast<uint8_t>(Register::kRwMinutes), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetHour(uint8_t hour) {
  if (hour > 23) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    hour = 23;
  }

  uint8_t buffer = (((hour / 10) << 4) | (hour % 10));

  if (!WriteRegister(static_cast<uint8_t>(Register::kRwHours), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetDay(uint8_t day) {
  if (day > 31) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    day = 31;
  } else if (day < 1) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    day = 1;
  }

  uint8_t buffer = (((day / 10) << 4) | (day % 10));

  if (!WriteRegister(static_cast<uint8_t>(Register::kRwDays), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetWeek(Week week) {
  if (!WriteRegister(static_cast<uint8_t>(Register::kRwWeekdays),
          static_cast<uint8_t>(week))) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetMonth(uint8_t month) {
  if (month > 12) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    month = 12;
  } else if (month < 1) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    month = 1;
  }

  uint8_t buffer = 0;

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwCenturyMonths), &buffer)) {
    return false;
  }

  buffer = (buffer & 0B11100000) | (((month / 10) << 4) | (month % 10));

  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwCenturyMonths), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetYear(uint8_t year) {
  if (year > 99) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    year = 99;
  }

  uint8_t buffer = (((year / 10) << 4) | (year % 10));

  if (!WriteRegister(static_cast<uint8_t>(Register::kRwYears), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::SetTime(Time time) {
  if (time.second > 59) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.second = 59;
  }
  if (time.minute > 59) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.minute = 59;
  }
  if (time.hour > 23) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.hour = 23;
  }
  if (time.day > 31) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.day = 31;
  } else if (time.day < 1) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.day = 1;
  }
  if (time.month > 12) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.month = 12;
  } else if (time.month < 1) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.month = 1;
  }
  if (time.year > 99) {
    LogMessage(LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
    time.year = 99;
  }

  uint8_t buffer[7] = {0};

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwVlSeconds), &buffer[0])) {
    return false;
  }

  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwCenturyMonths), &buffer[5])) {
    return false;
  }

  buffer[0] = (buffer[0] & 0B10000000) |
              (((time.second / 10) << 4) | (time.second % 10));
  buffer[1] = (((time.minute / 10) << 4) | (time.minute % 10));
  buffer[2] = (((time.hour / 10) << 4) | (time.hour % 10));
  buffer[3] = (((time.day / 10) << 4) | (time.day % 10));
  buffer[4] = static_cast<uint8_t>(time.week);
  buffer[5] =
      (buffer[5] & 0B11100000) | (((time.month / 10) << 4) | (time.month % 10));
  buffer[6] = (((time.year / 10) << 4) | (time.year % 10));

  for (uint8_t i = 0; i < 7; i++) {
    if (!WriteRegister(
            static_cast<uint8_t>(
                static_cast<uint8_t>(Register::kRwVlSeconds) + i),
            buffer[i])) {
      return false;
    }
  }

  return true;
}

bool Pcf8563x::StopTimer() {
  uint8_t buffer = 0;

  // 关闭定时器
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwTimerControl), buffer)) {
    return false;
  }

  // 清除定时器TF标志位并关闭定时器TIE外部中断
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }
  buffer &= 0B11111010;
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), buffer)) {
    return false;
  }
  return true;
}

bool Pcf8563x::RunTimer(uint8_t n_value, TimerFreq freq_hz) {
  if (!WriteRegister(static_cast<uint8_t>(Register::kRwTimer), n_value)) {
    return false;
  }

  uint8_t buffer = 0B10000000 | static_cast<uint8_t>(freq_hz);

  // 开启定时器并设置频率
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwTimerControl), buffer)) {
    return false;
  }

  // 清除定时器TF标志位并开启定时器TIE外部中断
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }
  buffer = (buffer & 0B11111010) | 0B00000001;
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::CheckTimerFlag() {
  uint8_t buffer = 0;
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }

  return (buffer & 0B00000100) >> 2;
}

bool Pcf8563x::ClearTimerFlag() {
  uint8_t buffer = 0;
  // 清除定时器TF标志位
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }
  buffer &= 0B11111011;
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::StopScheduledAlarm() {
  uint8_t buffer = 0B10000000;

  // 关闭报警
  for (uint8_t i = 0; i < 4; i++) {
    if (!WriteRegister(
            static_cast<uint8_t>(
                static_cast<uint8_t>(Register::kRwMinuteAlarm) + i),
            buffer)) {
      return false;
    }
  }

  // 清除报警AF标志位并关闭报警AIE外部中断
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }
  buffer &= 0B11110101;
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::RunScheduledAlarm(TimeAlarm alarm) {
  uint8_t buffer = 0;

  if (alarm.minute.alarm_flag) {
    if (alarm.minute.value > 59) {
      LogMessage(
          LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
      alarm.minute.value = 59;
    }
    buffer = (((alarm.minute.value / 10) << 4) | (alarm.minute.value % 10));
  } else {
    buffer = 0B10000000;
  }
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwMinuteAlarm), buffer)) {
    return false;
  }

  if (alarm.hour.alarm_flag) {
    if (alarm.hour.value > 23) {
      LogMessage(
          LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
      alarm.hour.value = 23;
    }
    buffer = (((alarm.hour.value / 10) << 4) | (alarm.hour.value % 10));
  } else {
    buffer = 0B10000000;
  }
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwHourAlarm), buffer)) {
    return false;
  }

  if (alarm.day.alarm_flag) {
    if (alarm.day.value > 31) {
      LogMessage(
          LogLevel::kWarning, __FILE__, __LINE__, "Value out of range\n");
      alarm.day.value = 31;
    } else if (alarm.day.value < 1) {
      alarm.day.value = 1;
    }
    buffer = (((alarm.day.value / 10) << 4) | (alarm.day.value % 10));
  } else {
    buffer = 0B10000000;
  }
  if (!WriteRegister(static_cast<uint8_t>(Register::kRwDayAlarm), buffer)) {
    return false;
  }

  if (alarm.week.alarm_flag) {
    buffer = static_cast<uint8_t>(alarm.week.value);
  } else {
    buffer = 0B10000000;
  }
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwWeekdayAlarm), buffer)) {
    return false;
  }

  // 清除预定时间报警AF标志位并开启预定时间报警AIE外部中断
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }
  buffer = (buffer & 0B11110101) | 0B00000010;
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::CheckScheduledAlarmFlag() {
  uint8_t buffer = 0;
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }

  return (buffer & 0B00001000) >> 3;
}

bool Pcf8563x::ClearScheduledAlarmFlag() {
  uint8_t buffer = 0;
  // 清除预定时间报警AF标志位
  if (!ReadRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), &buffer)) {
    return false;
  }
  buffer &= 0B11110111;
  if (!WriteRegister(
          static_cast<uint8_t>(Register::kRwControlStatus2), buffer)) {
    return false;
  }

  return true;
}

bool Pcf8563x::ReadRegister(uint8_t reg, uint8_t* data, size_t length) {
  const uint8_t register_packet[] = {reg};

  if (bus_ != nullptr &&
      bus_->WriteRead(register_packet, sizeof(register_packet), data, length)) {
    return true;
  }
  LogMessage(LogLevel::kError, __FILE__, __LINE__,
      "PCF8563x register read failed (register: %#X)\n",
      static_cast<unsigned>(reg));
  return false;
}

bool Pcf8563x::WriteRegister(uint8_t reg, uint8_t value) {
  const uint8_t register_packet[] = {reg, value};

  if (bus_ != nullptr &&
      bus_->Write(register_packet, sizeof(register_packet))) {
    return true;
  }
  LogMessage(LogLevel::kError, __FILE__, __LINE__,
      "PCF8563x register write failed (register: %#X)\n",
      static_cast<unsigned>(reg));
  return false;
}
}  // namespace cpp_bus_driver
