...
uint8_t Sy6970::GetChipId() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwDeviceControl), &buffer)) {
...
bool Sy6970::SetHizModeEnable(bool enable) {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwInputSourceControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwInputSourceControl), buffer)) {
...
bool Sy6970::SetBoostHotThreshold(uint8_t threshold) {
  ...
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRwTemperatureMonitorControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetAdcConversionEnable(bool enable) {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwSystemControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwSystemControl), buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Write failed\n");
    return false;
  }
...
bool Sy6970::SetBatteryLoadEnable(bool enable) {
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRwPowerOnConfiguration), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetPumpxEnable(bool enable) {
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRwChargeCurrentControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetPrechargeCurrentLimit(uint16_t current_ma) {
  ...
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRwPrechrgTermCurrentControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetChargeVoltageLimit(uint16_t voltage_mv) {
  ...
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRwChargeVoltageControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetChargeTerminationEnable(bool enable) {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwChargeTerminationTimerControl),
          &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwChargeTerminationTimerControl),
          buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Write failed\n");
    return false;
  }
...
bool Sy6970::SetBatteryCompensationResistance(uint8_t resistance_mohm) {
  ...
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRwIrCompensationControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetForceAdaptiveCurrentLimit(bool force) {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwMiscellaneousOperationControl),
          &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwMiscellaneousOperationControl),
          buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Write failed\n");
    return false;
  }
...
bool Sy6970::SetBoostVoltage(uint16_t voltage_mv) {
  ...
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwBoostModeControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwBoostModeControl), buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Write failed\n");
    return false;
  }
...
bool Sy6970::SetVindpmMode(bool absolute) {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRwVindpmControl), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwVindpmControl), buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Write failed\n");
    return false;
  }
...
uint8_t Sy6970::GetChipStatus() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdSystemStatus), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return static_cast<uint8_t>(-1);
  }
...
uint8_t Sy6970::GetIrqFlag() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdFaultStatus), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return static_cast<uint8_t>(-1);
  }
...
bool Sy6970::ReadThermalRegulationStatus() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdBatteryVoltage), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
uint16_t Sy6970::GetSystemVoltage() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdSystemVoltage), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return 0;
  }
...
uint32_t Sy6970::ReadNtcVoltagePercentage() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdNtcVoltage), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return 0;
  }
...
bool Sy6970::ReadBusConnectionStatus() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdBusVoltageStatus), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
uint16_t Sy6970::GetChargingCurrent() {
  uint8_t buffer = 0;
  if (!bus_->Read(static_cast<uint8_t>(Register::kRdChargeCurrent), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return 0;
  }
...
bool Sy6970::ReadVindpmStatus() {
  uint8_t buffer = 0;
  if (!bus_->Read(
          static_cast<uint8_t>(Register::kRdInputCurrentLimitStatus), &buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Read failed\n");
    return false;
  }
...
bool Sy6970::SetRegisterReset(bool reset) {
  ...
  if (!bus_->Write(static_cast<uint8_t>(Register::kRwDeviceControl), buffer)) {
    LogMessage(LogLevel::kError, __FILE__, __LINE__, "Write failed\n");
    return false;
  }
