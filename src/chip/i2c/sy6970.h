...
class Sy6970 final : public I2cChipBase {
 public:
  ...
  /**
   * @brief 设置PUMPX使能
   * @param enable true:使能PUMPX, false:禁用PUMPX
   * @return 设置成功返回 true，失败返回 false
   */
  bool SetPumpxEnable(bool enable);
  ...
  /**
   * @brief 设置STAT引脚使能
   * @param disable true:禁用STAT引脚, false:使能STAT引脚
   * @return 设置成功返回 true，失败返回 false
   */
  bool SetStatPinDisable(bool disable);
   ...
   */
  uint32_t ReadNtcVoltagePercentage();
