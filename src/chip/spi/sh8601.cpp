 ...
 * @LastEditTime: 2025-07-14 09:39:30
 * @License: GPL 3.0
 */
#include "sh8601.h"
...
{
    bool Sh8601::begin(int32_t freq_hz)
    {
    ...
    bool Sh8601::set_render_window(uint16_t x_start, uint16_t x_end, uint16_t y_start, uint16_t y_end)
    {
        x_start += _x_offset;
        x_end += _x_offset;
        y_start += _y_offset;
    ...
    bool Sh8601::send_color_stream(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *data)
    {
        // 有效性检查
    ...
    bool Sh8601::set_write_stream_mode(Write_Stream_Mode mode)
    {
        uint8_t buffer[4] = {0};
    ...
    bool Sh8601::set_brightness(uint8_t value)
    {
        uint8_t buffer[] =
            {
                static_cast<uint8_t>(Cmd::WO_WRITE_REGISTER),
    ...
    bool Sh8601::set_color_enhance(Color_Enhance mode)
    {
        uint8_t buffer[] =
            {
                static_cast<uint8_t>(Cmd::WO_WRITE_REGISTER),
    ...
    bool Sh8601::set_color_format(Color_Format format)
    {
        uint8_t buffer[] =
            {
                static_cast<uint8_t>(Cmd::WO_WRITE_REGISTER),
