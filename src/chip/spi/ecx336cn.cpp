...
bool Ecx336cn::SetPowerSaveMode(bool enable) {
  return WriteRegister(static_cast<uint8_t>(Register::kWoPowerSaveMode),
      static_cast<uint8_t>(enable ? 0x0E : 0x0F));
}

bool Ecx336cn::WriteRegister(uint8_t reg, uint8_t value) {
  const uint8_t packet[] = {reg, value};
  if (bus_ != nullptr && bus_->Write(packet, sizeof(packet))) {
    return true;
  }
  LogMessage(LogLevel::kError, __FILE__, __LINE__,
      "ECX336CN register write failed (register: %#X)\n",
