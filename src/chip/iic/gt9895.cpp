 ...
 * @LastEditTime: 2025-07-09 09:16:31
 * @License: GPL 3.0
 */
#include "gt9895.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool Gt9895::get_single_touch_point(Touch_Point &tp, uint8_t finger_num)
    {
        ...
        // 手指id读取从0开始
        uint8_t buffer_finger_id = (buffer_2[buffer_touch_point_offset] >> 4) + 1;
        // 如果这个手指id不为需要读取的话就读取失败
        if (buffer_finger_id != finger_num)
        {
            return false;
        }

        Touch_Info buffer_ti;
        buffer_ti.finger_id = buffer_finger_id;
        buffer_ti.x = (buffer_2[buffer_touch_point_offset + 2] | static_cast<uint16_t>(buffer_2[buffer_touch_point_offset + 3]) << 8) * _touch_x_y_scale_factor;
        buffer_ti.y = (buffer_2[buffer_touch_point_offset + 4] | static_cast<uint16_t>(buffer_2[buffer_touch_point_offset + 5]) << 8) * _touch_x_y_scale_factor;
    ...
    bool Gt9895::get_multiple_touch_point(Touch_Point &tp)
    {
        ...
        // 如果手指数为0或者大于最大触摸手指数
        if ((buffer_2[2] == 0) || (buffer_2[2] > MAX_TOUCH_FINGER_COUNT))
        {
            return false;
        }
        tp.finger_count = buffer_2[2];
